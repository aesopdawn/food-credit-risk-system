...
> 覆盖范围：评分引擎 + 涉企事件增删改 + 登录鉴权与三级角色 + 企业档案增删改 四阶段成果。

---

## 一、系统概述

面向**市场监管**场景的食品企业信用风险分类管理系统。归集企业基础信息、行政处罚、抽查检查、投诉举报、信用修复等多维涉企数据，通过**标准化指标 + 确定性评分引擎**自动完成企业信用风险 **A/B/C/D 四级**动态评级，并提供风险监测、动态预警、分级监管、数据统计、AI 智能研判等能力。
...
| 智能研判 | AI 对话助手 + 一键生成研判报告 | ✅ |
| 动态预警 | 评级落 C/D 或一票否决自动生成预警 | ✅ |
| 分级监管 | 按 A/B/C/D 给出差异化监管建议 | ✅（建议文案）|
| 数据统计 | 总览页统计 + 图表 | ✅ |
| 台账管理 | 企业档案 / 事件 / 评级历史的增删改查 | ✅（导出待做）|
| 标准化指标 + 评分模型 | 5 维度确定性评分引擎 | ✅ |
| 早发现/早提醒/早处置 | 事件录入即重评级 + 联动预警（处置闭环待做） | ◑ |
| 结果共享应用 | 当前本地库；云端共享待做 | ◻ |
...
flowchart TD
  ...
  V -->|写操作| SA["Server Actions<br/>app/actions/*"]
  V -->|AI 请求| API["API 路由<br/>api/chat · api/report"]
  SA -->|增删改后| RATE["lib/rating.ts<br/>重新评级"]
...
**三类"写"入口**：
- **Server Actions**（`app/actions/*`）—— 登录、企业 CRUD、事件 CRUD。客户端组件直接 `await` 调用，写库后 `revalidatePath` 让相关页面**自动刷新**，无需手写 `router.refresh()`。
...
erDiagram
  ...
  RiskEvent {
    ...
    bool   isVeto "一票否决"
    date   occurredAt
    string payload "JSON 文本"
  }
  RatingRecord {
    float  score "0-100"
    string level "A/B/C/D"
    string breakdown "JSON 文本: 各维度明细"
    date   computedAt
  }
  Alert {
    string level "高/中/低"
...
- **统一事件表 `RiskEvent`**：所有涉企事件（处罚/抽检/投诉/修复/许可）归一到一张表，是评分引擎的**唯一数据源**。
- **评级历史 `RatingRecord`**：每次评级追加一条，保留历史 → 支撑"评级走势图"与台账。
...
| 信用修复 REPAIR | 加 CREDIT 维（不超满分） | `+ severity × 2` |
...
- **一票否决**：任一 `isVeto` 事件（严重食品安全事故）→ 直接定 **D 级**，分数压到 ≤ 40。
- 输出 `breakdown`（每维度扣分明细）+ `topRisks`（扣分最多的若干项）→ 支撑"为什么是这个等级"的解释与 AI 研判。

### 自动重新评级机制（`lib/rating.ts` → `reRateEnterprise`）

让评级"活"起来的关键。**任何对企业 / 事件的增删改之后都会调用**：

```
取该企业全部事件 → 跑 scoring 引擎 → 追加一条 RatingRecord（留历史）→ 联动刷新预警
```

预警联动：评级落 C/D 或一票否决时，若当前无对应级别"待处置"预警则自动生成（幂等，不重复建警）。
...
- **信用风险评级卡**：等级 / 得分 / 分级监管建议；可「重新评级」；可「AI 生成研判报告」。
- **各维度得分明细**：每维度进度条 + 扣分项。
- **评级走势**：基于 RatingRecord 历史的折线图（含 A/B/C 阈值参考线）。
- **主要风险因素** + **涉企风险事件时间线**（可录入/编辑/删除事件）。
- 文件：`app/enterprises/[id]/page.tsx` → `components/EnterpriseDetailView.tsx`（+ `EnterpriseFormModal`、`RiskEventFormModal`）。
...
- 表格展示全部预警（级别/企业/行业/区县/原因/状态/时间），可按"待处置/已处置"过滤。
- 预警来源：① seed 初始化；② 评级落 C/D 或一票否决时由 `reRateEnterprise` 自动生成。
- 文件：`app/alerts/page.tsx` → `components/AlertsView.tsx`。
- 现状：预警仅"生成"，人工"处置改状态"的闭环为后续 TODO。

### 5.7 AI 智能研判

| 用法 | 实现 | 说明 |
|---|---|---|
| **对话 Agent** | `app/api/chat/route.ts` | `streamText` + 工具调用（`stopWhen: stepCountIs(6)`）。4 个工具 `listEnterprises`/`getEnterpriseProfile`/`explainRating`/`getStatistics` **全部查真实数据库**再作答。前端右下角悬浮 `ChatWidget`。 |
| **研判报告** | `app/api/report/route.ts` | `generateText` 基于企业数据生成"总体结论 + 风险点 + 监管建议"报告。 |
...
- **造数据**：`prisma/seed.ts` 用随机数生成 80 家企业 + 涉企事件 + 评级 + 预警，并建 3 个演示账号。因用 `Math.random()`，**每人 seed 出的数据各不相同**（仅登录账号一致）。
- **克隆后首次运行**：
...
**已实现**：评分引擎 · 监管总览 · 企业名录 · 企业详情 · 企业档案 CRUD · 涉企事件 CRUD · 自动重新评级 · 评级走势图 · 动态预警生成 · 登录鉴权 + 三级角色 · AI 对话助手 · AI 研判报告。

**后续 TODO**（详见 `CLAUDE.md` 第 10 节）：
- 台账导出 Excel / PDF
- 分级监管：按等级自动生成抽查计划
- 预警"处置"操作闭环（改状态 + 记录处置意见）
