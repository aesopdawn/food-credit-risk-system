...
- **两遍计算**：先按 `severity × 系数 × 时间衰减` 扣分（行政处罚/抽查检查/投诉举报），再处理信用修复加分。
- **信用修复(REPAIR)加分**：把分**加回到指定的「修复对象」维度**（行政处罚/抽查/投诉，存于 `payload.修复对象`；未指定则自动回补当前扣分最多的维度），恢复分值 = `力度(severity) × 2`，**封顶不超过该维度满分**。扣分与修复明细并存（`breakdown[].deductions` / `breakdown[].repairs`）→ **分数可升可降且不删除原扣分记录**。
- 阈值：A≥85，B≥70，C≥60，否则 D。
- **一票否决**：`isVeto` 事件（严重食品安全事故）直接定 D 级，**信用修复也抬不动**。
- `breakdown` 记录每维度扣分/修复明细 → 支撑"为什么是这个等级"的解释与 AI 研判。
